class RapidproStartFlowTask < ApplicationTask
  ...
  def start_flow!(phone_call)
    response = rapidpro_client.start_flow!(rapidpro_start_flow_params)
  ...
  def max_flows_to_start
    (ENV["RAPIDPRO_START_FLOW_TASK_MAX_FLOWS_TO_START"] || DEFAULT_MAX_FLOWS_TO_START).to_i
  end

  def rapidpro_start_flow_params
    @rapidpro_start_flow_params ||= JSON.parse(ENV["RAPIDPRO_START_FLOW_TASK_REMOTE_REQUEST_PARAMS"] || "{}")
  end
